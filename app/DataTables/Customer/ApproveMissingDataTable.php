...
class ApproveMissingDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('ac_name', function ($query) {
                ...
                return view('customer.accounts.components.input', compact('query', 'name'));
            })
            ->editColumn('ac_phone', function ($query) {
                $name = 'ac_phone';
                $input_type = 'number';

                return view('customer.accounts.components.input',
                    compact('query', 'name', 'input_type'));
            })
    ...
    public function query(ProjectAccounts $model)
    {
        return $model->with(['project'])
                     ...
                     ->where(['authorization_request' => 1, 'authorization_status' => 2])
                     ->newQuery()
