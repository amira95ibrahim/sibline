...
class RefuseAccountsDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('authorization_status', function ($query) {
                ...
                return view('customer.accounts.components.radio-buttns', compact('query', 'val'));
            })
            ->editColumn('authorization_comment', function ($query) {
                $val = $query->authorization_comment;

                return view('customer.accounts.components.comment', compact('query', 'val'));
            })
     ...
     *
     * @param  \App\Models\Customer/RefuseAccount $model
    ...
    public function query(ProjectAccounts $model)
    {
        return $model->with(['project'])
                     ...
                     ->where(['authorization_request' => 1, 'authorization_status' => 4])
                     ->newQuery()
